...
if p=="FAQ":
    ...
    if st.button("Go"):
        if q=="Find the publisher with most books published":
            ...
            mycursor.execute("""SELECT BOOK_AUTHORS,COUNT(BOOK_TITLE) AS BOOK_COUNT  FROM BOOKS 
                             WHERE BOOK_AUTHORS IS NOT NULL AND BOOK_AUTHORS!='NA'
                            GROUP BY BOOK_AUTHORS ORDER BY COUNT(BOOK_TITLE) DESC LIMIT 3""" )
             ...
             mycursor.execute("""SELECT CATEGORIES,AVG(PAGECOUNT) AS AVERAGE_PAGE_COUNT
                ...
                mycursor.execute("""SELECT PUBLICATION_YEAR FROM BOOKS
                 ...
                 mycursor.execute("""SELECT BOOK_AUTHORS,BOOK_PUBLISHER,PUBLICATION_YEAR,COUNT(ID) AS COUNT_OF_BOOKS FROM BOOKS
                                  WHERE BOOK_AUTHORS !='NA' AND BOOK_PUBLISHER!='NA' AND PUBLICATION_YEAR!='NA'
                               GROUP BY BOOK_PUBLISHER ,PUBLICATION_YEAR ,BOOK_AUTHORS HAVING  COUNT(ID)>1
                               """ ) 
